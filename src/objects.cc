...
// static
MaybeHandle<Object>
Object::ConvertToIndex(Isolate *isolate, Handle<Object> input,
...
CreateListFromArrayLikeFastPath(Isolate *isolate, Handle<Object> object,
...
// static
MaybeHandle<FixedArray>
Object::CreateListFromArrayLike(Isolate *isolate, Handle<Object> object,
                                ElementTypes element_types) {
...
// static
MaybeHandle<Object>
JSProxy::GetProperty(Isolate *isolate, Handle<JSProxy> proxy, Handle<Name> name,
...
Handle<Object> JSReceiver::GetDataProperty(LookupIterator *it) {
  for (; it->IsFound(); it->Next()) {
    ...
    case LookupIterator::ACCESS_CHECK:
      ...
      if (it->isolate()->context() != nullptr && it->HasAccess())
        continue;
      V8_FALLTHROUGH;
    case LookupIterator::JSPROXY:
      it->NotFound();
...
void JSObject::EnsureWritableFastElements(Handle<JSObject> object) {
  ...
  if (raw_elems->map() != ReadOnlyRoots(heap).fixed_cow_array_map())
    return;
...
MaybeHandle<Object>
Object::GetPropertyWithDefinedGetter(Handle<Object> receiver,
...
MaybeHandle<Object>
JSObject::GetPropertyWithFailedAccessCheck(LookupIterator *it) {
  Isolate *isolate = it->isolate();
...
JSObject::GetPropertyAttributesWithFailedAccessCheck(LookupIterator *it) {
  Isolate *isolate = it->isolate();
  Handle<JSObject> checked = it->GetHolder<JSObject>();
  Handle<InterceptorInfo> interceptor =
      it->GetInterceptorForFailedAccessCheck();
...
V8_WARN_UNUSED_RESULT Maybe<bool>
FastAssign(Handle<JSReceiver> target, Handle<Object> source,
           const ScopedVector<Handle<Object>> *excluded_properties,
           bool use_set) {
...
MaybeHandle<Object>
Object::ArraySpeciesConstructor(Isolate *isolate,
                                Handle<Object> original_array) {
...
Object::SpeciesConstructor(Isolate *isolate, Handle<JSReceiver> recv,
...
Map::GetConstructorFunction(Handle<Map> map, Handle<Context> native_context) {
...
namespace {
std::pair<MaybeHandle<JSFunction>, Handle<String>>
GetConstructorHelper(Handle<JSReceiver> receiver) {
  Isolate *isolate = receiver->GetIsolate();
...
// static
MaybeHandle<JSFunction>
JSReceiver::GetConstructor(Handle<JSReceiver> receiver) {
...
void MigrateFastToSlow(Handle<JSObject> object, Handle<Map> new_map,
  ...
  if (inobject_properties) {
    Heap *heap = isolate->heap();
    heap->ClearRecordedSlotRange(object->address() +
...
Maybe<bool> Object::SetSuperProperty(LookupIterator *it, Handle<Object> value,
  ...
  for (; own_lookup.IsFound(); own_lookup.Next()) {
    ...
    case LookupIterator::JSPROXY: {
      ...
      if (!owned.FromJust()) {
        return JSReceiver::CreateDataProperty(&own_lookup, value, should_throw);
      }
...
Maybe<bool> Object::CannotCreateProperty(Isolate *isolate,
  ...
  RETURN_FAILURE(isolate, should_throw,
                 NewTypeError(MessageTemplate::kStrictCannotCreateProperty,
                              name, Object::TypeOf(isolate, receiver),
                              receiver));
}
...
void Map::EnsureDescriptorSlack(Isolate *isolate, Handle<Map> map, int slack) {
  ...
  if (slack <= descriptors->NumberOfSlackDescriptors())
    return;
...
template <class T>
static int
AppendUniqueCallbacks(Isolate *isolate, Handle<TemplateList> callbacks,
                      Handle<typename T::Array> array, int valid_descriptors) {
...
// static
MaybeHandle<Context>
JSBoundFunction::GetFunctionRealm(Handle<JSBoundFunction> function) {
...
MaybeHandle<Object>
JSObject::SetOwnElementIgnoreAttributes(Handle<JSObject> object, uint32_t index,
                                        Handle<Object> value,
                                        PropertyAttributes attributes) {
  Isolate *isolate = object->GetIsolate();
...
Maybe<PropertyAttributes>
JSObject::GetPropertyAttributesWithInterceptor(LookupIterator *it) {
    ...
    case LookupIterator::ACCESSOR: {
      if (!it->IsConfigurable()) {
        ...
        if (is_strict(language_mode)) {
          isolate->Throw(*isolate->factory()->NewTypeError(
              MessageTemplate::kStrictDeleteProperty, it->GetName(), receiver));
          return Nothing<bool>();
        }
        return Just(false);
      }
...
bool JSObject::ReferencesObjectFromElements(FixedArray *elements,
  ...
  if (IsObjectElementsKind(kind) || kind == FAST_STRING_WRAPPER_ELEMENTS) {
    ...
    for (int i = 0; i < length; ++i) {
      ...
      if (!element->IsTheHole(isolate) && element == object)
        return true;
    }
  } else {
...
#undef TYPED_ARRAY_CASE
  ...
  case SLOW_STRING_WRAPPER_ELEMENTS: {
    ...
    if (ReferencesObjectFromElements(elements, kind, obj))
      return true;
    break;
  }
  ...
  case SLOW_SLOPPY_ARGUMENTS_ELEMENTS: {
    ...
    for (uint32_t i = 0; i < elements->parameter_map_length(); ++i) {
      ...
      if (!value->IsTheHole(heap->isolate()) && value == obj)
        return true;
    }
    // Check the arguments.
    FixedArray *arguments = elements->arguments();
    kind =
        arguments->IsNumberDictionary() ? DICTIONARY_ELEMENTS : HOLEY_ELEMENTS;
    if (ReferencesObjectFromElements(arguments, kind, obj))
      return true;
    break;
  }
  case NO_ELEMENTS:
...
Maybe<bool>
JSObject::PreventExtensionsWithTransition(Handle<JSObject> object,
                                          ShouldThrow should_throw) {
...
// static
MaybeHandle<Object>
JSReceiver::OrdinaryToPrimitive(Handle<JSReceiver> receiver,
...
V8_WARN_UNUSED_RESULT Maybe<bool>
FastGetOwnValuesOrEntries(Isolate *isolate, Handle<JSReceiver> receiver,
                          bool get_entries, Handle<FixedArray> *result) {
  ...
  Handle<FixedArray> enum_keys_store =
  ...
  for (int index = 0; index < number_of_own_descriptors; index++) {
    ...
    if (!next_key->IsSymbol()) {
      enum_keys_store->set(count, *next_key);
      FieldIndex field_index = FieldIndex::ForDescriptor(*map, index);
      indices->set(count, Smi::FromInt(field_index.GetLoadByFieldIndex()));
    }
...
// marking logic.
Handle<Map>
Map::AddMissingTransitions(Isolate *isolate, Handle<Map> split_map,
                           Handle<DescriptorArray> descriptors,
...
Handle<Map> Map::CopyAsElementsKind(Isolate *isolate, Handle<Map> map,
  ...
  DCHECK_IMPLIES(!map->CanHaveFastTransitionableElementsKind(),
                 IsDictionaryElementsKind(kind) ||
                     IsTerminalElementsKind(kind));
...
DescriptorArray::CopyForFastObjectClone(Isolate *isolate,
                                        Handle<DescriptorArray> src,
                                        int enumeration_index, int slack) {
  if (enumeration_index + slack == 0) {
    return isolate->factory()->empty_descriptor_array();
  }
...
WeakArrayList *PrototypeUsers::Compact(Handle<WeakArrayList> array, Heap *heap,
  ...
  for (int i = kFirstIndex; i < array->length(); i++) {
    ...
    if (element->IsSmi())
      continue;
    if (element->IsClearedWeakHeapObject())
      continue;
...
FlatStringReader::FlatStringReader(Isolate *isolate, Vector<const char> input)
    : Relocatable(isolate), str_(0), is_one_byte_(true),
      length_(input.length()), start_(input.start()) {}
...
template <> class RawStringComparator<uint16_t, uint16_t> {
public:
...
template <> class RawStringComparator<uint8_t, uint8_t> {
public:
...
template <> bool String::IsEqualTo(Vector<const uint8_t> str) {
  return IsOneByteEqualTo(str);
}

template <> bool String::IsEqualTo(Vector<const uc16> str) {
...
bool String::IsOneByteEqualTo(Vector<const uint8_t> str) {
  ...
  for (int i = 0; i < slen; i++) {
    if (Get(i) != static_cast<uint16_t>(str[i]))
      return false;
  }
  return true;
}
...
Handle<String>
NativeCodeFunctionSourceString(Handle<SharedFunctionInfo> shared_info) {
...
Script::FindSharedFunctionInfo(Isolate *isolate, const FunctionLiteral *fun) {
...
SharedFunctionInfo::wasm_exported_function_data() const {
  DCHECK(HasWasmExportedFunctionData());
...
SharedFunctionInfo::ScriptIterator::ScriptIterator(
    ...
    : isolate_(isolate), shared_function_infos_(shared_function_infos),
      index_(0) {}
...
// static
Handle<Object>
SharedFunctionInfo::GetSourceCode(Handle<SharedFunctionInfo> shared) {
  Isolate *isolate = shared->GetIsolate();
...
// static
Handle<Object>
SharedFunctionInfo::GetSourceCodeHarmony(Handle<SharedFunctionInfo> shared) {
  Isolate *isolate = shared->GetIsolate();
  if (!shared->HasSourceCode())
    return isolate->factory()->undefined_value();
  Handle<String> script_source(
...
namespace {
template <typename Code> void DropStackFrameCacheCommon(Code *code) {
  ...
  for (int i = 0; i < deopt_count; i++) {
    ...
    while (iterator.HasNext() &&
      ...
      case Translation::DOUBLE_REGISTER: {
        int reg_code = iterator.Next();
        os << "{input="
           << RegisterConfiguration::Default()->GetDoubleRegisterName(reg_code)
           << "}";
        break;
      }
...
DependentCode::InsertWeakCode(Isolate *isolate, Handle<DependentCode> entries,
                              DependencyGroup group, MaybeObjectHandle code) {
...
Handle<DependentCode>
DependentCode::EnsureSpace(Isolate *isolate, Handle<DependentCode> entries) {
  if (entries->Compact())
    return entries;
...
public:
  ...
  StringSharedKey(Handle<String> source, Handle<SharedFunctionInfo> shared,
      ...
      : HashTableKey(CompilationCacheShape::StringSharedHash(
        ...
        source_(source), shared_(shared), language_mode_(language_mode),
        position_(position) {}
...
template <typename Derived, typename Shape>
Handle<Derived>
HashTable<Derived, Shape>::New(Isolate *isolate, int at_least_space_for,
                               PretenureFlag pretenure,
...
template <typename Derived, typename Shape>
Handle<Derived>
HashTable<Derived, Shape>::NewInternal(Isolate *isolate, int capacity,
                                       PretenureFlag pretenure) {
  Factory *factory = isolate->factory();
...
MaybeHandle<SharedFunctionInfo>
CompilationCacheTable::LookupScript(Handle<String> src,
                                    Handle<Context> native_context,
                                    LanguageMode language_mode) {
...
CompilationCacheTable::Put(Handle<CompilationCacheTable> cache,
                           Handle<String> src,
                           Handle<SharedFunctionInfo> shared,
                           LanguageMode language_mode, Handle<Object> value) {
...
template <typename Derived, typename Shape>
Handle<Derived>
Dictionary<Derived, Shape>::DeleteEntry(Isolate *isolate,
...
template <typename Derived, typename Shape>
Handle<Derived>
Dictionary<Derived, Shape>::Add(Isolate *isolate, Handle<Derived> dictionary,
                                Key key, Handle<Object> value,
                                PropertyDetails details, int *entry_out) {
...
SimpleNumberDictionary::Set(Isolate *isolate,
...
NumberDictionary::Set(Isolate *isolate, Handle<NumberDictionary> dictionary,
                      uint32_t key, Handle<Object> value,
                      Handle<JSObject> dictionary_holder,
                      PropertyDetails details) {
...
Handle<PropertyCell>
PropertyCell::InvalidateEntry(Isolate *isolate,
...
Handle<PropertyCell>
PropertyCell::PrepareForValue(Isolate *isolate,
                              Handle<GlobalDictionary> dictionary, int entry,
...
bool JSReceiver::HasProxyInPrototype(Isolate *isolate) {
  for (PrototypeIterator iter(isolate, this, kStartAtReceiver,
    ...
    if (iter.GetCurrent<Object>()->IsJSProxy())
      return true;
  }
  return false;
}
...
MaybeHandle<Name>
FunctionTemplateInfo::TryGetCachedPropertyName(Isolate *isolate,
                                               Handle<Object> getter) {
  if (getter->IsFunctionTemplateInfo()) {
...
template void
HashTable<GlobalDictionary, GlobalDictionaryShape>::Rehash(Isolate *isolate);
